    ...
    <>
      ...
      >
        <ModalBody className="p-0">
          ...
          {/* ── Two-Pane Body ── */}
          <div className="block lg:flex lg:flex-row max-h-[85vh] lg:min-h-[520px] overflow-y-auto lg:overflow-hidden">

            {/* ── Left Main Panel ── */}
            <div className="lg:flex-1 min-w-0 lg:overflow-y-auto p-5 lg:p-8 block space-y-8">
                ...
                {isEditingTitle ? (
                  <div className="flex items-start gap-2">
                    <input type="text" autoFocus
                      className="flex-1 text-2xl font-semibold bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-indigo-500 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                      value={title}
                    ...
                    <Button
                      color="primary"
                      size="sm"
                      className="shrink-0 inline-flex items-center justify-center w-10 h-10 p-0"
                  ...
                  <div className="group flex items-start gap-3">
                    <h2 className="text-2xl lg:text-[28px] font-semibold text-slate-900 dark:text-white leading-tight">
                      {task.title}
                    </h2>
                    <Button
                      color="light"
                      size="sm"
                      className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0 w-8 h-8 inline-flex items-center justify-center mt-1"
                ...
                {isEditingDesc ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button type="button" color="primary" onClick={handleSaveDesc} disabled={isSaving}
                        className="
                      inline-flex items-center gap-1.5
                      px-4 py-3 rounded-lg text-xs font-semibold
                      transition-colors
                    ">
                        {isSaving ? <Spinner size="sm" /> : "Save"}
                      </Button>
                      <Button type="button" color="light" onClick={() => setIsEditingDesc(false)}
                        className="px-3.5 py-3 text-xs font-semibold  rounded-lg transition">
                        Cancel
                      </Button>
                    </div>
                  </div>
                  ...
                  <div onClick={() => setIsEditingDesc(true)}
                    className="p-4 rounded-xl border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/70 cursor-pointer transition-colors shadow-sm">
                    {task.desc || task.description ? (
                      <p className="text-[14px] text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed mb-0">
                        {task.desc || task.description}
                      </p>
                    ) : (
              ...
              {subtasks.length > 0 && (
                <div className="block space-y-2">
                  ...
                  {subtasks.map((st) => (
                    <div key={st.id}
                      className="flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 group shadow-sm">
            ...
            {/* ── Right Sidebar ── */}
            <div className="w-full lg:w-[300px] xl:w-[340px] shrink-0 lg:overflow-y-auto border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 p-5 lg:p-6 block space-y-6 min-h-0">
              ...
              <div className="block space-y-5">
                ...
                <FieldRow label="Due Date">
                  <DatePicker
                    ...
                    className="w-full bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/70 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 min-h-[36px] text-xs font-semibold text-slate-700 dark:text-slate-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-colors shadow-sm"
              ...
              <hr className="border-slate-200 dark:border-slate-800 m-0" />
              ...
              {/* Metadata Footer */}
              <div className="block space-y-2.5 text-[11px] pt-1">
                <div className="flex items-center justify-between">
