...
const Menu = ({ data, className }) => {
  ...
  let currentLink = function(selector){
      ...
      elm.forEach(function(item){
          ...
          if (activeRouterLink) {
              let parents = getParents(item,`.sidebar-menu-sm`, 'nk-menu-item');
  ...
  let routeChange = function(e){
      let selector = document.querySelectorAll(".sidebar-menu-link-alt")
      selector.forEach((item, index)=>{
          if(item.classList.contains('active')){
              ...
              currentLink(`.sidebar-menu-link-alt`);
          }
      })
  }
  ...
  return (
    <ul className={`nk-menu sidebar-menu-sm ${className ? className : ''}` }>
      {data.map((item, index) =>
        <Fragment key={index}>
            {item.heading ? (
              ...
              <li className={classNames({'nk-menu-item': true, 'has-sub' : item.subMenu})}>
                {!item.subMenu ? (
                  <NavLink to={item.link} className="nk-menu-link sidebar-menu-link-alt" target={item.newTab && '_blank'}>
                  ...
                  <>
                    <a href="#" className="nk-menu-link sidebar-menu-link-alt nk-menu-toggle" onClick={menuToggle}>
                      {item.icon  && <span className="nk-menu-icon">
                        <Icon name={item.icon} />
                      </span>}
                      <span className="nk-menu-text">{item.text}</span>
                      ...
                      <ul className="nk-menu-sub">
                        {item.subMenu.map((sItem, sIndex) =>
                          <li className={classNames({'nk-menu-item': true, 'has-sub' : sItem.subMenu})} key={sIndex}>
                              {!sItem.subMenu ? (
                                <NavLink to={sItem.link} className="nk-menu-link sidebar-menu-link-alt" target={sItem.newTab && '_blank'}>
                                    ...
                                    <ul className="nk-menu-sub">
                                      {sItem.subMenu.map((s2Item, s2Index) =>
                                        <li className={classNames({'nk-menu-item': true, 'has-sub' : s2Item.subMenu})} key={s2Index}>
                                            {!s2Item.subMenu ? (
                                              <NavLink to={s2Item.link} className="nk-menu-link sidebar-menu-link-alt" target={s2Item.newTab && '_blank'}>
                                                  ...
                                                  <ul className="nk-menu-sub">
                                                    {s2Item.subMenu.map((s3Item, s3Index) =>
                                                      <li className="nk-menu-item" key={s3Index}>
                                                          <NavLink to={s3Item.link} className="nk-menu-link sidebar-menu-link-alt" target={s3Item.newTab && '_blank'}>
